...
let () =
  ...
  in let (peoples,choices) = from_preferences_file infile in 
  (* let ()= debug_lire_choices choices in
     let ()= debug_lire_peoples peoples in *)
  let graph = build_graph peoples choices in
  let string_graph = gmap graph (fun x -> (string_of_int x)) in
  let () = export begin_graph string_graph in
  let (fl2,fgraph)= construction_gap_solution graph 0 1 0 in
  let graph = construct_flow_solution graph fgraph in
  let list_peoples = results_to_list_peoples graph peoples choices in
  let list_sport = results_to_list_sport graph peoples choices in
  let graph = gmap graph (fun x -> (string_of_int x)) in
  let () = export_all_to_txt outfile list_peoples list_sport choices in
